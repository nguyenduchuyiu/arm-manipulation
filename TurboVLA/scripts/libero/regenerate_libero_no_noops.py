...
def main(args):
    ...
    if os.path.isdir(args.libero_target_dir) and os.listdir(args.libero_target_dir) and not args.overwrite:
        raise FileExistsError(
            f"target directory is not empty: {args.libero_target_dir}; pass --overwrite to replace generated files"
        )
    os.makedirs(args.libero_target_dir, exist_ok=True)

    # Prepare JSON file to record success/false and initial states per episode
    metainfo_json_dict = {}
    metainfo_json_out_path = os.path.join(args.libero_target_dir, "metainfo.json")
