...
Run: python scripts/record_pick_episode.py
"""
...
def run_pick_episode(
    ...
    print(f"episode: {out/'pick_episode.npz'}")
    return success
...
def main() -> None:
    ...
    if not run_pick_episode(scene_path=args.scene, output_dir=args.out):
        raise RuntimeError("Pick episode did not reach success")
